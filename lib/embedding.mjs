...
//
//   GEMINI_API_KEY or GOOGLE_API_KEY -> Gemini `gemini-embedding-2`
...
import { rateLimitGemini } from "./gemini-rate-limit.mjs";
...
async function geminiEmbedBatch(texts) {
  ...
  if (!apiKey) throw new Error("Set GEMINI_API_KEY or GOOGLE_API_KEY to use Gemini embeddings.");
  ...
  const response = await rateLimitGemini(() =>
    ...
    // Batch embedding quotas may count every input content. Charge each input
    // against the limiter so the KB build is safe under that stricter model.
    { cost: texts.length },
  );
